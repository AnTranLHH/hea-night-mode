...
public class FloatVariableToText : MonoBehaviour
{
    ...
    [SerializeField]
    private bool _userDateTimeFormatFromCode = true;
    [SerializeField]
    ...
    private void SetValueText(float value)
    {
        if (_convertToDateTime)
        {
            ...
            string dateTimeText = dateTimeText = string.Format(_additionFormat, ts.ToString(@_dateTimeFormat));
            if (_userDateTimeFormatFromCode)
            {
                dateTimeText = $"{ts.Days}D {ts.Hours}H {ts.Minutes}M";
            }
            _textMesh.text = dateTimeText;
            return;
        }
